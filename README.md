...
1. Install all required dependencies:
   ...
   ```
2. Populate the environment variables (e.g. locally via `.env` or system var exports). At minimum, `API_ID`, `API_HASH`, and `BOT_TOKEN` must all be present. `API_ID` must be a real numeric app id from `my.telegram.org` and cannot be `0`. If you deploy on Koyeb and miss `API_ID` or `API_HASH`, or set an invalid `API_ID`, Pyrogram will fail during startup with an authorization error.
